...
def do_registration_stats(root_target_dir):
    ...
    for core_name in core_names:
        ...
        graph = RegGraph.from_config_path(config_path, remove_additional_data=False, filter_ids=[2,3,6,7])    
        paths = get_multiomics_integration_path()
        df = pd.DataFrame()
        best_tre = np.inf
        best_warped_section = None
        best_path = None
        print(paths)
        for reg_seq in paths:
            ...
            if mean_tre < best_tre:
                best_tre = mean_tre
                best_warped_section = warped_section.copy()
                best_path =  reg_seq
        warped_dir = join(target_dir, 'warped_section')      
        best_warped_section.store(warped_dir)
        print(best_path, best_tre)
...
def main():
    ...
    root_dir = '/mnt/scratch/maximilw/multiomics_integration/multiomics_registration_exhaustive'
