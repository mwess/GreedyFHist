...
def main2():
    ...
    # root_dir = '../save_directories/pairwise_analysis/greedy_f_hist_params_sigma_test_all_cores'
    # root_dir = '../save_directories/pairwise_analysis/greedy_f_hist_params_no_msf_def'
    root_dir = '../save_directories/pairwise_analysis/greedy_f_hist_params_standard_no_masks_no_def_rev_all_info'
    if not os.path.exists(root_dir):
        os.mkdir(root_dir)
    # core_names = get_core_names()
    df_all = pd.DataFrame()
    ...
    for core_name in core_names:
        ...
        for i in range(len(section_ids) -1):
            ...
            for rep in range(N_ROUNDS):
                ...
                options.greedy_opts.s2 = 5
                options.deformable_do_denoising = False
                options.temporary_directory = 'all_rev'
...
def main3():
    ...
    # root_dir = '../save_directories/pairwise_analysis/greedy_f_hist_params_sigma_test_all_cores'
    # root_dir = '../save_directories/pairwise_analysis/greedy_f_hist_params_no_msf_def'
    root_dir = '../save_directories/pairwise_analysis/greedy_f_hist_params_standard_no_def_rev'
    if not os.path.exists(root_dir):
        os.mkdir(root_dir)
    # core_names = get_core_names()
    df_all = pd.DataFrame()
    ...
    for core_name in core_names:
        ...
        for i in range(len(section_ids) -1):
            ...
            for rep in range(N_ROUNDS):
                ...
                create_if_not_exists(options.output_directory)
                start = time.time()                
                mask = source_section.get_annotations_by_names("tissue_mask")
                moving_mask = mask.data if mask is not None else None
                mask = target_section.get_annotations_by_names("tissue_mask")
                fixed_mask = mask.data if mask is not None else None
