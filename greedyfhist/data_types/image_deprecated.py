...
class ImageDepr:
    ...
    def transform_data(self, registerer: GreedyFHist, transformation: RegistrationTransforms) -> 'ImageDepr':
    ...
    @classmethod
    def load_data_from_config(cls, dct: dict) -> 'ImageDepr':
        path = dct['path']
        is_annotation = dct.get('is_annotation', False)
        return ImageDepr.load_from_path(path, is_annotation)
    
    @classmethod
    def load_from_path(cls, path: str, is_annotation: bool = False) -> 'ImageDepr':
